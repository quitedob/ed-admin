    ...
    children: [
      {
        path: 'works',
        name: 'AigcWorks',
        component: () => import('@/views/aigc/works/index.vue'),
        meta: { title: 'AIGC作品' }
      },
      {
        path: 'gallery',
        name: 'AigcGallery',
        component: () => import('@/views/aigc/gallery/index.vue'),
        meta: { title: '作品墙' }
      }
    ]
  },
...
router.beforeEach(async (to, from, next) => {
  ...
  if (!userStore.getMenuList || userStore.getMenuList.length === 0) {
    // 用户数据未初始化，重新初始化
    userStore.mockLogin()
    createNewRouter(userStore.routerList)
    // 重新尝试导航
    return next(to)
  }
...
export function createNewRouter(data) {
  ...
  menuList.forEach(menu => {
    const parentRouteName = `route_${menu.id}`
    const childRouteName = menu.id.toString()

    if (allRoutes.some(route => route.name === parentRouteName)) {
      router.removeRoute(parentRouteName)
    }
    if (allRoutes.some(route => route.name === childRouteName)) {
      router.removeRoute(childRouteName)
    }
  })
