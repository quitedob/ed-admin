...
function getQuillInstance() {
  // 通过 DOM 查找 Quill 实例
  const editorElement = document.querySelector('.editor-wrapper .ql-container')
  if (editorElement && editorElement.__quill) {
    return editorElement.__quill
  }
  
  // 备用方法：通过 Quill 全局实例查找
  if (window.Quill) {
    const editors = document.querySelectorAll('.ql-editor')
    for (const editor of editors) {
      if (editor.parentElement && editor.parentElement.__quill) {
        return editor.parentElement.__quill
      }
    }
  }
