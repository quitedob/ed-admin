  ...
  <div class="lecturer-list-container">
    ...
    <div class="top-bar">
      ...
      <div class="action-buttons">
        <el-button v-if="hasPermission('lecturer:create')" type="primary" @click="handleCreateLecturer">
          <el-icon><Plus /></el-icon>
          新建老师
        </el-button>
        <el-button
          v-if="hasPermission('lecturer:batch-delete')"
          type="danger"
        ...
        <el-table-column label="操作" width="340" fixed="right">
          <template #default="scope">
            <div class="table-actions">
              ...
              <el-button v-if="hasPermission('lecturer:authorize-resource')" link type="primary" size="small" @click="handleAuthorizeManagement(scope.row)">
                授权管理
              </el-button>
              <el-button v-if="hasPermission('lecturer:view-detail')" link type="primary" size="small" @click="handleViewDetail(scope.row)">
                查看详情
              </el-button>
              <el-button v-if="hasPermission('lecturer:delete')" link type="danger" size="small" @click="handleDelete(scope.row)">
                删除
              </el-button>
            </div>
          </template>
    ...
    <UnifiedAuthorizeDrawer
      ...
      :initial-authorized-resources="selectedLecturer?.authorizedResources || { courses: [], assignments: [], exams: [] }"
      @save="handleUnifiedAuthorizeUpdate"
    />
...
const handleAuthorizeManagement = (lecturer) => {
  selectedLecturer.value = { ...lecturer }
  authorizeDialogVisible.value = true
}

const handleUnifiedAuthorizeUpdate = (authorizationData) => {
  if (selectedLecturer.value) {
    const index = lecturers.value.findIndex(l => l.id === selectedLecturer.value.id)
    if (index > -1) {
      // 更新班级授权
      lecturers.value[index].authorizedClasses = authorizationData.authorizedClasses
      lecturers.value[index].stats.classCount = authorizationData.authorizedClasses.length

      // 更新资源授权
      lecturers.value[index].authorizedResources = authorizationData.authorizedResources

      // 计算总资源数量
      const totalResources = (authorizationData.authorizedResources.courses?.length || 0) +
                           (authorizationData.authorizedResources.assignments?.length || 0) +
                           (authorizationData.authorizedResources.exams?.length || 0)

      ElMessage.success(`授权更新成功：${authorizationData.authorizedClasses.length}个班级，${totalResources}个资源`)
    }
  }
