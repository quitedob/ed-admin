            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="handleStatus(scope.row)">
                  <el-button v-if="scope.row.statusId == 0" v-permission="'website:link:edit'" text type="primary">启用</el-button>
                  <el-button v-if="scope.row.statusId == 1" v-permission="'website:link:edit'" text type="primary">禁用</el-button>
                </el-dropdown-item>
                <el-dropdown-item>
...
<script setup>
  ...
  onMounted(() => {
    console.log(`打开文件: ${location.pathname} -> views/common/link/index.vue`);
  });
