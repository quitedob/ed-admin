  ...
  <div id="exam-create-container" class="exam-create-container">
    ...
    <div id="exam-main-content" class="main-content">
      <el-form ref="formRef" :model="examForm" :rules="rules" label-width="120px" id="exam-form">
        ...
        <div v-show="currentStep === 0" id="step-basic-info" class="step-content">
          <el-card shadow="never" id="basic-info-card">
            ...
            <el-form-item label="学科" prop="subject">
              <el-select v-model="examForm.subject" placeholder="请选择学科" clearable>
                <el-option
        ...
        <!-- 第三步：组建试卷 -->
        <div v-show="currentStep === 2" id="step-paper-builder" class="step-content paper-builder-step">
          <PaperBuilder
        ...
        <div v-show="currentStep === 4" id="step-exam-settings" class="step-content">
          <el-card shadow="never" id="exam-settings-card">
            ...
            <!-- 手动发布时的说明 -->
            <el-form-item
              v-if="examForm.settings.showScoreMode === 'manual'"
              label="手动发布说明"
            >
              <div class="manual-release-info">
                <el-alert
                  title="手动发布模式"
                  type="info"
                  show-icon
                ...
                >
                  <template #default>
                    <p>选择手动发布模式后，学生完成考试后不会立即看到成绩。</p>
                    <p>您需要在考试管理页面手动点击"发布成绩"按钮，学生才能查看自己的考试成绩。</p>
                    <p>这适用于需要人工复核或统一公布成绩的场景。</p>
                  </template>
                </el-alert>
              </div>
            </el-form-item>
            ...
            <el-form-item label="屏幕禁止切屏">
              <el-switch v-model="examForm.settings.disableTabSwitch" />
              <div class="form-tip">禁止学生切换浏览器标签页，违者将自动提交试卷</div>
            </el-form-item>

            <el-form-item label="强制全屏模式">
              <el-switch v-model="examForm.settings.forceFullscreen" />
              <div class="form-tip">要求学生必须全屏答题，退出全屏将自动提交试卷</div>
            </el-form-item>
          </el-card>
        </div>
      </el-form>
    ...
    <QuestionSelectorDialog
      ...
      :initial-selected-questions="examForm.questionBanks"
...
const handleNextStep = () => {
  // 验证当前步骤
  if (currentStep.value === 0) {
    formRef.value.validateField(['title', 'classId'], (valid) => {
      if (valid) {
        currentStep.value++
      }
    })
...
const handleQuestionSelect = (selectedQuestions) => {
  // 过滤掉已经存在的题目
  const existingIds = examForm.value.questionBanks.map(q => q.id)
  const newQuestions = selectedQuestions.filter(q => !existingIds.includes(q.id))

  // 添加新题目，设置默认分值
  const questionsWithScore = newQuestions.map(q => ({
    ...q,
    score: q.score || getDefaultScore(q.type)
  }))

  examForm.value.questionBanks = [...examForm.value.questionBanks, ...questionsWithScore]
  updateTotalScore()

  ElMessage.success(`成功添加 ${questionsWithScore.length} 道题目到试卷`)
}
...
const updateTotalScore = () => {
  const total = examForm.value.questionBanks.reduce((sum, q) => sum + (q.score || 0), 0)
  examForm.value.totalScore = total
}
