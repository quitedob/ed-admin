...
<script setup>
  ...
  onMounted(async () => {
    // Redirect to modern edit course page - 使用新的符合规范页面
    if (route.query.courseId) {
      router.replace({ path: '/course/update/modern', query: { courseId: route.query.courseId } })
      return
    }

    // 如果没有courseId，则创建新课程
    if (!route.query.courseId) {
      router.push({ path: '/course/update/modern' })
      return
    }
  })
  const handleClose = () => {
