<template>
        ...
        <el-card>
          ...
          <el-form :model="courseData.basicInfo" label-width="120px">
            ...
            <el-form-item label="学科分类" prop="subject">
              <el-select v-model="courseData.basicInfo.subject" placeholder="请选择学科分类" style="width: 100%" clearable>
                <el-option
                  ...
                  <div class="section-content">
                    ...
                    <template v-if="section.contentType === 'programming'">
                      ...
                      <div class="programming-section">
                        ...
                        <!-- 编程作业绑定 -->
                        <el-divider content-position="left">
                          <span class="divider-title">编程作业绑定</span>
                        </el-divider>

                        <el-form-item label="绑定编程作业">
                          <div class="homework-binding">
                            <div class="selected-homeworks">
                              <el-tag
                                v-for="homework in section.selectedProgrammingHomeworks"
                                :key="homework.id"
                                closable
                                @close="removeProgrammingHomework(chapterIndex, sectionIndex, homework.id)"
                                style="margin: 4px;"
                              >
                            ...
                            <div class="homework-actions">
                              <el-button
                                type="primary"
                                size="small"
                                @click="openProgrammingHomeworkSelector(chapterIndex, sectionIndex)"
                              >
                                <el-icon><Plus /></el-icon>
                                选择编程作业
                              </el-button>
                              <el-button
                                v-if="section.selectedProgrammingHomeworks?.length > 0"
                                type="danger"
                                size="small"
                                @click="clearProgrammingHomeworks(chapterIndex, sectionIndex)"
                              >
                                <el-icon><Delete /></el-icon>
                                清空作业
                              </el-button>
                          ...
                          <div class="form-tip">
                            ...
                            <span>选择与编程内容相关的作业，学生完成编程练习后可获得相应作业记录。</span>
                          </div>
                        </el-form-item>

                        <!-- 已绑定编程作业的详情显示 -->
                        <div v-if="section.selectedProgrammingHomeworks?.length > 0" class="homework-details">
                          <el-divider content-position="left">已绑定编程作业详情</el-divider>
                            ...
                            >
                              <div class="homework-info">
                                <div class="homework-header">
                                  ...
                                  <el-tag :type="getTypeTagType(homework.type)" size="small">
                                    {{ getTypeLabel(homework.type) }}
                                  </el-tag>
                                </div>
                                <p class="homework-desc">{{ homework.description || '暂无描述' }}</p>
                                <div class="homework-meta">
                                  <span><el-icon><User /></el-icon> {{ homework.className }}</span>
                                  <span><el-icon><Document /></el-icon> {{ homework.courseName }}</span>
                                  <span><el-icon><EditPen /></el-icon> {{ homework.questions?.length || 0 }}道题</span>
                                </div>
                              </div>
                            </el-card>
                          </div>
                    ...
                    <template v-if="section.contentType === 'homework'">
                      ...
                      <div class="homework-section-config">
                        ...
                        <el-form-item label="绑定作业" prop="homeworkIds">
                          <div class="homework-binding">
                            <div class="selected-homeworks">
                              <el-tag
                                v-for="homework in section.selectedHomeworks"
                                :key="homework.id"
                                closable
                                @close="removeHomework(chapterIndex, sectionIndex, homework.id)"
                                style="margin: 4px;"
                              >
                                {{ homework.title }} ({{ homework.id }})
                              </el-tag>
                            ...
                            <div class="homework-actions">
                              <el-button
                                type="primary"
                                size="small"
                                @click="openHomeworkSelector(chapterIndex, sectionIndex)"
                              >
                                <el-icon><Plus /></el-icon>
                                选择作业
                              </el-button>
                              <el-button
                                v-if="section.selectedHomeworks?.length > 0"
                                type="danger"
                                size="small"
                                @click="clearHomeworks(chapterIndex, sectionIndex)"
                              >
                                <el-icon><Delete /></el-icon>
                                清空作业
                              </el-button>
                          ...
                          <div class="form-tip">
                            <el-icon><InfoFilled /></el-icon>
                            <span>从作业管理系统选择已创建的作业进行绑定，学生完成作业后进度将自动同步。</span>
                          </div>
                        </el-form-item>

                        <!-- 已绑定作业的详情显示 -->
                        <div v-if="section.selectedHomeworks?.length > 0" class="homework-details">
                          <el-divider content-position="left">已绑定作业详情</el-divider>
                          <div class="homework-cards">
                            <el-card
                              v-for="homework in section.selectedHomeworks"
                              :key="homework.id"
                    ...
                    <!-- 练习作业内容 -->
                    <template v-if="section.contentType === 'practice'">
                      <div class="resource-item">
                        <div class="resource-header">
                          <el-icon class="resource-icon"><EditPen /></el-icon>
                          <span class="resource-title">题目管理</span>
                          <div class="resource-stats">
                            <el-tag type="info" size="small">共 {{ section.selectedQuestions.length }} 道题目</el-tag>
                            <el-button type="primary" size="small" @click="openQuestionSelector(chapterIndex, sectionIndex)">
                              <el-icon><Plus /></el-icon>
                              从题库选择题目
                            </el-button>
                            <el-button size="small" @click="clearQuestionsFromSection(chapterIndex, sectionIndex)" :disabled="section.selectedQuestions.length === 0">
                              <el-icon><Delete /></el-icon>
                              清空题目
                            </el-button>
                          </div>
                        ...
                        <div class="resource-content">
                          <div v-if="section.selectedQuestions.length > 0" class="questions-list">
                            <div
                              v-for="(question, qIndex) in section.selectedQuestions"
                              :key="question.id"
                              class="question-item"
                            >
                              <div class="question-header">
                                ...
                                <div class="question-actions">
                                  ...
                                  <el-button size="small" @click="moveQuestionDown(chapterIndex, sectionIndex, qIndex)" :disabled="qIndex === section.selectedQuestions.length - 1">
                                    <el-icon><ArrowDown /></el-icon>
                                  </el-button>
                                  <el-button size="small" type="danger" @click="removeQuestionFromSection(chapterIndex, sectionIndex, qIndex)">
                                    <el-icon><Delete /></el-icon>
                                  </el-button>
                                </div>
                              </div>
  ...
  <!-- 操作按钮 -->
    ...
    <!-- 作业选择对话框 -->
    <SelectHomeworkDialog
      v-model="showHomeworkSelector"
      :preselected-ids="currentHomeworkIds"
      @confirm="handleHomeworkSelect"
    />

    <!-- 题目选择器对话框 -->
    <QuestionSelectorDialog
      v-model="questionSelectorVisible"
      :initial-selected-questions="currentSelectedQuestions"
...
// 作业选择相关数据
const currentSelectingSection = ref({ chapterIndex: -1, sectionIndex: -1, type: '' })
const currentHomeworkIds = ref([])
...
const currentSelectedQuestions = ref([])
const currentTargetQuestionSection = ref({ chapterIndex: -1, sectionIndex: -1 })
...
const addSection = (chapterIndex) => {
  ...
  const newSection = {
    ...
    homeworkIds: [],
    selectedHomeworks: [],
    programmingHomeworkIds: [],
    selectedProgrammingHomeworks: [],
    // 练习作业选中的题目
    selectedQuestions: [],
    // 练习题内容（非作业小节）
    hasPractice: false,
    ...
    sections: chapter.sections.map(section => {
      ...
      switch (section.contentType) {
        ...
        case 'homework':
          ...
          // 添加直接关联的题目ID
          sectionData.questionIds = section.questionIds || ''
...
// 题目相关处理函数
const validateQuestions = (questionIds) => {
  if (!questionIds) {
    ElMessage.warning('请先输入题目ID')
    return
  }

  const ids = questionIds.split(/[,，]/).map(s => s.trim()).filter(s => s)
  ...
  // 模拟校验逻辑 - 这里应该调用API检查题目是否存在
  const invalidIds = []
  const validIds = []

  ids.forEach(id => {
    // 模拟检查 - 假设以q_开头的ID都是有效的
    if (id.startsWith('q_') || id.startsWith('p_') || /^\d+$/.test(id)) {
      validIds.push(id)
    } else {
      invalidIds.push(id)
    }
  ...
  if (invalidIds.length > 0) {
    ElMessage.error(`以下题目ID格式不正确: ${invalidIds.join(', ')}`)
  } else {
    ElMessage.success(`校验通过，共关联 ${validIds.length} 道题目`)
  }
}

// 打开题库选择对话框
const openQuestionBankDialog = (chapterIndex, sectionIndex) => {
  // 这里可以打开一个题库选择对话框
  ElMessage.info('题库选择功能开发中...')
...
const moveQuestionUp = (chapterIndex, sectionIndex, questionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  const questionList = section.selectedQuestions || section.practice?.questionList || []
  if (questionIndex > 0) {
    const temp = questionList[questionIndex]
    questionList[questionIndex] = questionList[questionIndex - 1]
    questionList[questionIndex - 1] = temp

    // 更新排序
    if (section.practice?.questionList) {
      section.practice.questionList.forEach((question, idx) => {
        question.order = idx + 1
      })
    }
...
// 作业选择相关方法
const openHomeworkSelector = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  currentSelectingSection.value = { chapterIndex, sectionIndex, type: 'homework' }

  // 设置当前已选择的作业ID
  currentHomeworkIds.value = section.selectedHomeworks?.map(hw => hw.id) || []

  showHomeworkSelector.value = true
}

const openProgrammingHomeworkSelector = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  currentSelectingSection.value = { chapterIndex, sectionIndex, type: 'programming' }

  // 设置当前已选择的作业ID
  currentHomeworkIds.value = section.selectedProgrammingHomeworks?.map(hw => hw.id) || []

  showHomeworkSelector.value = true
}

const handleHomeworkSelect = (selectedHomeworks) => {
  const { chapterIndex, sectionIndex, type } = currentSelectingSection.value
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]

  if (type === 'homework') {
    // 为章节作业类型绑定作业
    section.selectedHomeworks = selectedHomeworks
    section.homeworkIds = selectedHomeworks.map(hw => hw.id)
    ElMessage.success(`成功绑定 ${selectedHomeworks.length} 个作业到章节`)
  } else if (type === 'programming') {
    // 为编程内容绑定作业
    section.selectedProgrammingHomeworks = selectedHomeworks
    section.programmingHomeworkIds = selectedHomeworks.map(hw => hw.id)
    ElMessage.success(`成功绑定 ${selectedHomeworks.length} 个编程作业`)
  }

  // 重置选择状态
  currentSelectingSection.value = { chapterIndex: -1, sectionIndex: -1, type: '' }
  currentHomeworkIds.value = []
}

// 题目选择相关方法
const openQuestionSelector = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  currentTargetQuestionSection.value = { chapterIndex, sectionIndex }
  currentSelectedQuestions.value = section.selectedQuestions || []
  questionSelectorVisible.value = true
}

const handleQuestionSelect = (selectedQuestions) => {
  const { chapterIndex, sectionIndex } = currentTargetQuestionSection.value
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  ...
  // 重置选择状态
  currentTargetQuestionSection.value = { chapterIndex: -1, sectionIndex: -1 }
  currentSelectedQuestions.value = []
  questionSelectorVisible.value = false

  ElMessage.success(`成功选择 ${selectedQuestions.length} 道题目`)
}

const removeQuestionFromSection = (chapterIndex, sectionIndex, questionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedQuestions.splice(questionIndex, 1)
  ElMessage.success('题目已移除')
}

const clearQuestionsFromSection = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedQuestions = []
  ElMessage.success('题目已清空')
}
...
const removeHomework = (chapterIndex, sectionIndex, homeworkId) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedHomeworks = section.selectedHomeworks.filter(hw => hw.id !== homeworkId)
  section.homeworkIds = section.selectedHomeworks.map(hw => hw.id)
}

const removeProgrammingHomework = (chapterIndex, sectionIndex, homeworkId) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedProgrammingHomeworks = section.selectedProgrammingHomeworks.filter(hw => hw.id !== homeworkId)
  section.programmingHomeworkIds = section.selectedProgrammingHomeworks.map(hw => hw.id)
}

const clearHomeworks = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedHomeworks = []
  section.homeworkIds = []
  ElMessage.success('已清空绑定的作业')
}

const clearProgrammingHomeworks = (chapterIndex, sectionIndex) => {
  const section = courseData.chapters[chapterIndex].sections[sectionIndex]
  section.selectedProgrammingHomeworks = []
  section.programmingHomeworkIds = []
  ElMessage.success('已清空绑定的编程作业')
}
