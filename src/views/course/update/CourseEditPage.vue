            ...
            >
              <div class="chapter-header">
                ...
                <div class="chapter-actions">
                  <!-- 章节开放控制 -->
                  <el-button
                    v-if="canManageChapterOpenness"
                    link
                    size="small"
                    @click="toggleChapterOpenness(chapter)"
                    :class="chapter.isOpen ? 'open-button' : 'close-button'"
                  >
                    <el-icon>
                      <Unlock v-if="chapter.isOpen" />
                      <Lock v-else />
                    </el-icon>
                    {{ chapter.isOpen ? '已开放' : '已关闭' }}
                  </el-button>
              ...
              <div class="periods-list">
                ...
                >
                  ...
                  <div class="period-actions">
                    <!-- 小节开放控制 -->
                    <el-button
                      v-if="canManageSectionOpenness"
                      link
                      size="small"
                      @click="toggleSectionOpenness(period)"
                      :class="period.isOpen ? 'open-button' : 'close-button'"
                    >
                      <el-icon>
                        <Unlock v-if="period.isOpen" />
                        <Lock v-else />
                      </el-icon>
                      {{ period.isOpen ? '开放' : '关闭' }}
                    </el-button>
...
// 权限检查计算属性
const canManageChapterOpenness = computed(() => hasPermission('course:chapter:open'))
const canManageSectionOpenness = computed(() => hasPermission('course:section:open'))
...
const initCourse = async () => {
  ...
  try {
    ...
    chapters.value = chaptersData.map(chapter => ({
      ...chapter,
      isOpen: chapter.isOpen !== undefined ? chapter.isOpen : true, // 默认开放
      periods: (chapter.periods || []).map(period => ({
        ...period,
        isOpen: period.isOpen !== undefined ? period.isOpen : true // 默认开放
      }))
    }))
  } catch (error) {
...
// 切换章节开放状态
const toggleChapterOpenness = async (chapter) => {
  if (!canManageChapterOpenness.value) {
    ElMessage.warning('您没有权限管理章节开放状态')
    return
  }

  try {
    const newStatus = !chapter.isOpen
    chapter.isOpen = newStatus

    // 模拟API调用
    console.log('更新章节开放状态:', {
      chapterId: chapter.id,
      isOpen: newStatus
    })

    ElMessage.success(`章节已${newStatus ? '开放' : '关闭'}`)
  } catch (error) {
    // 恢复原状态
...
// 切换小节开放状态
const toggleSectionOpenness = async (period) => {
  if (!canManageSectionOpenness.value) {
    ElMessage.warning('您没有权限管理小节开放状态')
    return
  }

  try {
    const newStatus = !period.isOpen
    period.isOpen = newStatus

    // 模拟API调用
    console.log('更新小节开放状态:', {
      periodId: period.id,
      isOpen: newStatus
    })

    ElMessage.success(`小节已${newStatus ? '开放' : '关闭'}`)
  } catch (error) {
    // 恢复原状态
